...
class SentencesContainer:
    ...
    def __read(self):
        ...
        for line in lines:
            fields = line.split("*")
            if len(fields) != 2:
                raise Exception('Wrong format of line "{}"'.format(line))
    ...
    def addSentence(self, sentence, sound_fname):
        ...
        if ns in self.__sentences_map:
            raise Exception('Sentence "{}" is already in the container'.format(ns))
        ...
        while True:
            sound_fname_copy = "sound_{}.wav".format(random.randint(0, 1000000))
        ...
        with open(self.__path + "/data.txt", "a+") as f:
            ...
            text = "{}*{}".format(ns, sound_fname_copy)
            # print text
...
def play_sound(fname, start_pos):
    try:
        ...
        n_frames = (
            wave_file.getnframes() - n_frame_start + 1
...
class SaySentenceActionServer(object):
    def __init__(
        ...
        self._as = actionlib.SimpleActionServer(
            self._action_name,
            tiago_msgs.msg.SaySentenceAction,
    ...
    def execute_cb(self, goal):
        ...
        if sentence_uni.startswith(prefix):
            ...
            if sound_fname is None:
                print('using dialogflow for sentence "{}"'.format(sentence_uni))
                response, sound_params = detect_intent_text(
                    self._agent_name, "test_sess_012", ss.lower(), "pl", self._cred_file
                )
                sound_params = (
                    sound_params[0],
                    sound_params[1],
                ...
                print('using cached sentence "{}"'.format(sentence_uni))
                sound_params = (
                    sound_fname,
                    "keep",
                    prefix_time_length,
        ...
        print('Starting action for "' + sentence_uni + '"')
        success = True
        if best_d < 5:
            ...
            while (
                not self._playback_queue.finishedSoundId(sound_id)
        ...
        print('Ended action for "' + sentence_uni + '"', success)
...
def detect_intent_audio(
    project_id, session_id, audio_file_path, language_code, cred_file
):
    (transcript, confidence) = convert_audio_to_text(audio_file_path, language_code)
    return detect_intent_text(
        project_id, session_id, transcript, language_code, cred_file
    )


def detect_intent_from_audio(
    project_id, session_id, audio_file_path, language_code, cred_file
):
    ...
    voice = dialogflow.types.VoiceSelectionParams(name="pl-PL-Wavenet-B")

    synt = dialogflow.types.SynthesizeSpeechConfig(
        pitch=-10, speaking_rate=0.8, voice=voice
    )

    output_audio_config = dialogflow.types.OutputAudioConfig(
        audio_encoding=audio_encoding, synthesize_speech_config=synt
    )
...
def callback_common(response, sound_file, playback_queue):
    print("query_result:")
    print("query_text: {}".format(response.query_result.query_text))
    print("intent.name: {}".format(response.query_result.intent.name))
...
def callback(data, agent_name, playback_queue, cred_file):
    ...
    response, sound_file = detect_intent_text(
        agent_name, "test_sess_012", data.data, "pl", cred_file
    )
...
def callback_wav(data, agent_name, playback_queue, cred_file):
    ...
    response, sound_file = detect_intent_audio(
        agent_name, "test_sess_012", data.data, "pl", cred_file
    )
...
def listener():
    ...
    for sent, fname in izip(
        codecs.open(os.path.join(data_dir, "labels.txt"), encoding="utf-8"),
        open(os.path.join(data_dir, "files.txt")),
    ):
        ss = unicode(sent)
    ...
    say_as = SaySentenceActionServer(
        ...
        odm,
        sentence_dict,
        "text-to-speech-qtruau",
        cred_file_text_to_speech,
        sc,
    )
